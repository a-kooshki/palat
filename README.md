...
## تنظیمات PDF
- خروجی PDF به زبان انگلیسی تولید می‌شود.
- لوگو در بالای سمت چپ PDF نمایش داده می‌شود (اگر فایل `public/logo.png` وجود داشته باشد).
- QR Code با محتوای `agse.ir` در بالای سمت راست PDF قرار می‌گیرد.



## دسترسی از موبایل/دستگاه‌های دیگر در شبکه (LAN)
در نسخه نصبی برنامه، یک HTTP server داخلی روی ویندوز بالا می‌آید و برنامه علاوه بر خود Electron، از طریق مرورگر دستگاه‌های دیگر همان شبکه هم قابل دسترس است.

تمام کلاینت‌ها (خود برنامه + مرورگرهای شبکه) از یک فایل دیتای مشترک استفاده می‌کنند، بنابراین تغییرات بین همه مشترک است.

- پورت پیش‌فرض: `3210`
- آدرس دسترسی: `http://IP-WINDOWS:3210`
- نمونه: `http://192.168.1.20:3210`

نکته‌ها:
1. باید ویندوز و موبایل در یک شبکه باشند.
2. در Windows Firewall برای پورت `3210` دسترسی Inbound بدهید.
3. در صورت نیاز می‌توانید پورت را با متغیر محیطی `PALAT_LAN_PORT` تغییر دهید.
